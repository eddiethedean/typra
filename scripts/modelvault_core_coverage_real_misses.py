#!/usr/bin/env python3
"""Report *actual* uncovered source lines for modelvault-core from llvm-cov JSON.
...
Usage:
  cargo llvm-cov report -p modelvault-core --json --output-path target/coverage/modelvault-core.json
  python3 scripts/modelvault_core_coverage_real_misses.py --json target/coverage/modelvault-core.json
"""
...
def main() -> int:
    ...
    ap.add_argument(
        "--json",
        type=Path,
        default=Path("target/coverage/modelvault-core.json"),
        help="Path to llvm-cov JSON export (cargo llvm-cov report --json)",
    )
    ap.add_argument(
        "--out",
        type=Path,
        default=Path("target/coverage/modelvault-core-real-misses.txt"),
        help="Output path",
    )
    args = ap.parse_args()
    ...
    for f in files:
        ...
        if not rel.startswith("crates/modelvault-core/"):
            continue
    ...
    lines: list[str] = []
    lines.append(f"modelvault-core REAL uncovered source lines (total {total})\n\n")
