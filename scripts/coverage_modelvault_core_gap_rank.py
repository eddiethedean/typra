#!/usr/bin/env python3
"""Rank modelvault-core source files by uncovered lines (needs prior `cargo llvm-cov ... --lcov`).
...
def main() -> int:
    ...
    ap.add_argument(
        "--lcov",
        type=Path,
        default=Path("target/coverage/modelvault-core.lcov"),
        help="Path to LCOV for modelvault-core (default: target/coverage/modelvault-core.lcov)",
    )
    args = ap.parse_args()
    ...
    print(f"modelvault-core uncovered lines (total {total_missed})\n")
    for f, n in rows:
        if n == 0:
            continue
        fd = found[f]
    ...
    out = Path(os.environ.get("MODELVAULT_COVERAGE_GAP_TXT", "target/coverage/modelvault-core-gaps.txt"))
