...
def test_optional_field_omitted_and_null(tmp_path) -> None:
    db = modelvault.Database.open(str(tmp_path / "opt.modelvault"))
    fields = """[
      {"path": ["k"], "type": "string"},
...
def test_constraint_min_int_fails(tmp_path) -> None:
    db = modelvault.Database.open(str(tmp_path / "min.modelvault"))
    fields = """[
...
def test_enum_invalid_variant_fails(tmp_path) -> None:
    db = modelvault.Database.open(str(tmp_path / "enum.modelvault"))
    fields = """[
      {"path": ["id"], "type": "string"},
...
def test_unknown_top_level_key_fails(tmp_path) -> None:
    db = modelvault.Database.open(str(tmp_path / "unk.modelvault"))
