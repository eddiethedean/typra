...
def test_e2e_inventory_like_workflow_txn_query_reopen_compact_snapshot(
    ...
    path = tmp_path / "inventory.modelvault"
    snap = tmp_path / "inventory.snapshot.modelvault"
    compacted = tmp_path / "inventory.compacted.modelvault"
    ...
    conn2 = modelvault.dbapi.connect(str(path))
    cur2 = conn2.cursor()
    cur2.execute(
    ...
    db3 = modelvault.Database.open(str(compacted))
