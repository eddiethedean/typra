...
fn collection_name_for(_py: Python<'_>, cls: &Bound<'_, PyAny>) -> PyResult<String> {
    if let Ok(v) = cls.getattr("__modelvault_collection__") {
        if !v.is_none() {
...
fn is_none_type(py: Python<'_>, t: &Bound<'_, PyAny>) -> PyResult<bool> {
    // `types.NoneType` exists on Python 3.10+, but ModelVault supports Python 3.9+.
...
fn py_to_modelvault_type(py: Python<'_>, t: &Bound<'_, PyAny>, depth: usize) -> PyResult<Type> {
    if depth > 32 {
    ...
    if let Some(o) = origin.as_ref() {
        ...
        if o.is(&typing.getattr("Annotated")?) {
            ...
            return py_to_modelvault_type(py, &args[0], depth + 1);
        }
    }
    ...
    if let Some(o) = origin {
        ...
        if let Ok(list_t) = builtins.getattr("list") {
            if o.is(&list_t) {
                ...
                let inner = py_to_modelvault_type(py, &args[0], depth + 1)?;
...
fn field_constraints_from_annotated(
    py: Python<'_>,
    t: &Bound<'_, PyAny>,
) -> PyResult<Vec<modelvault_core::schema::Constraint>> {
    let (origin, args) = origin_and_args(py, t)?;
...
fn fields_from_model(
    ...
    for (k, v) in hints.iter() {
        ...
        let ty = py_to_modelvault_type(py, &v, depth + 1)?;
...
fn indexes_from_model(
    _py: Python<'_>,
    cls: &Bound<'_, PyAny>,
) -> PyResult<Vec<modelvault_core::schema::IndexDef>> {
    use modelvault_core::schema::{IndexDef, IndexKind};
    let mut out = Vec::new();
    let Ok(v) = cls.getattr("__modelvault_indexes__") else {
        return Ok(out);
    };
    if v.is_none() {
        return Ok(out);
    ...
    for item in list.iter() {
        let spec: IndexSpec = item.extract().map_err(|_| {
            PyValueError::new_err(
                "__modelvault_indexes__ must contain IndexSpec values from modelvault.models.index/unique",
            )
        })?;
        let name = spec
            .name
...
fn schema_to_indexes_json(
    _py: Python<'_>,
    indexes: &[modelvault_core::schema::IndexDef],
) -> PyResult<String> {
    use serde_json::json;
    let arr: Vec<_> = indexes
        ...
        .map(|idx| {
            ...
            let kind = match idx.kind {
                modelvault_core::schema::IndexKind::Unique => "unique",
                modelvault_core::schema::IndexKind::NonUnique => "index",
            };
