...
ModelVault is positioned as **the database for application models** — not a replacement for every datastore. Use this section to pick the right tool and explain ModelVault to your team.
...
|------------|------------|
| [ModelVault vs SQLite](sqlite.md) | You already know SQLite and want model-first ergonomics |
| [ModelVault vs JSON files](json.md) | You outgrew `data.json` |
| [ModelVault vs TinyDB](tinydb.md) | You want TinyDB-like simplicity with production structure |
| [ModelVault vs DuckDB](duckdb.md) | You are choosing between app storage and analytics |

## Hybrid patterns

ModelVault and DuckDB are **complementary**:

- **ModelVault** — authoritative app state (users, settings, domain records)
- **DuckDB** — analytics, exports, ad-hoc SQL on snapshots

Many teams use SQLite + DuckDB today; ModelVault + DuckDB follows the same split with a model-native OLTP side.
