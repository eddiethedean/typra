...
For *why* ModelVault exists and how it compares to SQLite or JSON files, start with [Why ModelVault](why_modelvault.md) and [Comparisons](../comparisons/index.md).

ModelVault’s model in six ideas. For implementation detail, see [Specifications](../specs/index.md). For release status, see the [roadmap](https://github.com/eddiethedean/modelvault/blob/main/ROADMAP.md).
...
|------|-----|-------------|
| **On-disk** | `Database.open("app.modelvault")` | Durable single file |
| **In-memory** | `Database.open_in_memory()` | Explicit snapshots only |

On open, ModelVault validates the file header, replays the **schema catalog**, rebuilds the latest row map from **record** segments, and restores **index** state.
...
ModelVault is **schema-first**: invalid states should be rejected **on write**, not discovered later in production.

## 4. Models — how you author schemas

| Language | Typical approach |
|----------|------------------|
| **Python** | `@dataclass` or Pydantic + **`modelvault.models`** (recommended) |
...
Errors are structured: nested **paths**, expected vs actual where applicable. Rust: `DbError::Validation`. Python: `ModelVaultValidationError` / `ValueError`.
...
A minimal read-only **SQL** subset exists for DB-API interop. Broader SQL is on the [roadmap](https://github.com/eddiethedean/modelvault/blob/main/ROADMAP.md#post-10-isoiec-9075-sql-track).

## File format (single file, versioned)

Everything lives in one **`.modelvault`** file: header, superblocks, append-only **segments**, checkpoints. Design reference: [On-disk format](../specs/on_disk_file_format.md).
