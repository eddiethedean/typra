...
Typra exists to answer one question:
...
Typra targets the **application model** niche directly.
...
3. **Single-file deployment** — one `.typra` file (or in-memory for tests).
...
|---------|----------------|
| Store application models directly | `typra.models.collection(db, YourModel)` |
...
| Single-file deploy | Copy `app.typra` with your binary or installer |
...
Typra is a strong fit when you want **typed documents with indexes** in one file. Be aware of current limits:

- **Not a SQL database** — model-first APIs are primary; SQL is a growing read-only subset today.
- **Not for analytics at scale** — use DuckDB (or export) for heavy OLAP; Typra is OLTP-oriented.
...
| Browser-only storage without native code | IndexedDB / WASM story (future for Typra) |

## Next steps

- [Quickstart](quickstart.md) — first model in five minutes
- [Comparisons](../comparisons/index.md) — Typra vs SQLite, JSON, TinyDB, DuckDB
