...
As of **v1.0.x**, Typra ships a **persisted schema catalog** (per-field **constraints** on catalog **v3**; **index definitions** on catalog **v4**), **secondary indexes**, richer **queries** (including `OR`, ranges, and `order_by`), **schema compatibility checks** + migration helpers, **record insert/get/delete** with **nested row values** (record payload **v2**; payload **v3** enables **multi-segment schema field paths**; v1 segments still replay), **engine validation** before append, **transactions** (`with db.transaction()` in Python / `Database::transaction` in Rust), **compaction**, **checkpoints** (faster reopen), in-memory databases + snapshots, and the first pieces of **bounded-memory query** scaffolding (ephemeral `Temp` spill segments; external sort plumbing behind `order_by`; spillable agg/join foundations).
