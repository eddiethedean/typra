...
Workspace crates and the PyPI distribution share **`[workspace.package] version`** in the root `Cargo.toml` (currently **0.15.2**). Bump that version when you cut releases, then tag **`vX.Y.Z`** to match.
