...
Today, the **on-disk** path supports open/create, superblocks, checksummed segments, manifest publication, a **persisted schema catalog** (including **secondary index** definitions on catalog **v4**), **record insert/get** (**v1** and **v2** payloads; new inserts use **v2** when needed), **index segment** replay, and a typed query path (equality + `And`/`Or` + ranges + **`limit`** + **`order_by`** + streaming `query_iter`; see [`guide_python.md`](guide_python.md)). **In-memory** mode is implemented via the same APIs (`Database::open_in_memory` / Python `open_in_memory`) with **snapshot** export/import. The first pieces of **bounded-memory execution** exist (`Temp` spill segments + external sort plumbing), but the full “hybrid/streaming” story is still evolving—see [`ROADMAP.md`](../ROADMAP.md). For where **`Store`**, **`FileStore`**, and **`VecStore`** sit in the Rust workspace, see [`03_rust_crate_and_module_layout.md`](03_rust_crate_and_module_layout.md).
