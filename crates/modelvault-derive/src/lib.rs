//! Procedural macro crate: derives [`modelvault_core::schema::DbModel`] for Rust types.
...
///
/// Intended for use with the `modelvault` facade crate’s **`derive`** feature.
...
pub fn derive_db_model(input: TokenStream) -> TokenStream {
    ...
    for f in &fields.named {
        ...
        let ty_expr = match rust_type_to_modelvault_type(&f.ty) {
            Ok(t) => t,
        ...
        field_defs.push(quote! {
            ::modelvault_core::FieldDef {
                path: ::modelvault_core::schema::FieldPath(vec![::std::borrow::Cow::Borrowed(#field_name)]),
                ty: #ty_expr,
        ...
        if is_unique {
            ...
            index_defs.push(quote! {
                ::modelvault_core::schema::IndexDef {
                    name: #idx_name.to_string(),
                    path: ::modelvault_core::schema::FieldPath(vec![::std::borrow::Cow::Borrowed(#field_name)]),
                    kind: ::modelvault_core::schema::IndexKind::Unique,
                }
            });
        } else if is_index {
            ...
            index_defs.push(quote! {
                ::modelvault_core::schema::IndexDef {
                    name: #idx_name.to_string(),
                    path: ::modelvault_core::schema::FieldPath(vec![::std::borrow::Cow::Borrowed(#field_name)]),
                    kind: ::modelvault_core::schema::IndexKind::NonUnique,
                }
            });
        }
    }
    ...
    let expanded = quote! {
        impl #impl_generics ::modelvault_core::DbModel for #name #ty_generics #where_clause {
            fn collection_name() -> &'static str {
                #collection_name
            }

            fn fields() -> ::std::vec::Vec<::modelvault_core::FieldDef> {
                ::std::vec![#(#field_defs),*]
            }
            ...
            fn indexes() -> ::std::vec::Vec<::modelvault_core::schema::IndexDef> {
                ::std::vec![#(#index_defs),*]
            }
        }
    };
...
fn rust_type_to_modelvault_type(ty: &Type) -> Result<proc_macro2::TokenStream, syn::Error> {
    match ty {
        Type::Path(p) => {
            let seg = p
                .path
            ...
            match (ident.as_str(), &seg.arguments) {
                ("bool", _) => Ok(quote! { ::modelvault_core::schema::Type::Bool }),
                ("i8" | "i16" | "i32" | "i64", _) => Ok(quote! { ::modelvault_core::schema::Type::Int64 }),
                ("u8" | "u16" | "u32" | "u64", _) => Ok(quote! { ::modelvault_core::schema::Type::Uint64 }),
                ("f32" | "f64", _) => Ok(quote! { ::modelvault_core::schema::Type::Float64 }),
                ("String", _) => Ok(quote! { ::modelvault_core::schema::Type::String }),
                ("Vec", PathArguments::AngleBracketed(args)) => {
                    ...
                    if is_u8(inner_ty) {
                        Ok(quote! { ::modelvault_core::schema::Type::Bytes })
                    } else {
                        let inner_expr = rust_type_to_modelvault_type(inner_ty)?;
                        Ok(quote! { ::modelvault_core::schema::Type::List(::std::boxed::Box::new(#inner_expr)) })
                    }
                }
                ("Option", PathArguments::AngleBracketed(args)) => {
                    ...
                    let inner_expr = rust_type_to_modelvault_type(inner_ty)?;
                    Ok(quote! { ::modelvault_core::schema::Type::Optional(::std::boxed::Box::new(#inner_expr)) })
                }
