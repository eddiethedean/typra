...
fn field_path_accepts_simple_path() {
    let path = FieldPath::new([std::borrow::Cow::Borrowed("profile"), std::borrow::Cow::Borrowed("timezone")])
        .expect("field path");
...
fn field_path_rejects_empty_segment() {
    let res = FieldPath::new([std::borrow::Cow::Borrowed("profile"), std::borrow::Cow::Borrowed("")]);
    assert!(matches!(res, Err(DbError::Schema(_))));
}
