//! Integration tests targeting llvm-cov "real miss" lines (see `scripts/typra_core_coverage_real_misses.py`).
...
fn sample_collection() -> CollectionInfo {
    CollectionInfo {
        id: CollectionId(1),
        name: "books".into(),
        current_version: typra_core::schema::SchemaVersion(1),
...
fn compact_to_hits_tracing_span_on_disk_db() {
    ...
    let dst = dir.path().join("dst.typra");
