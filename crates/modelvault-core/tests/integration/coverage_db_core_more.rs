...
use modelvault_core::schema::{DbModel, FieldDef, FieldPath, IndexDef, IndexKind, Type};
use modelvault_core::{CollectionId, Database, ScalarValue};
...
fn transaction_closure_error_rolls_back_written_rows() {
    ...
    let res = db.transaction(|d| {
        ...
        d.insert(cid, row)?;
        Err::<(), modelvault_core::DbError>(modelvault_core::DbError::NotImplemented)
    });
    assert!(res.is_err());
...
fn register_schema_version_with_indexes_inside_transaction_roundtrips() {
    ...
    let path = dir.path().join("schema_v.modelvault");
