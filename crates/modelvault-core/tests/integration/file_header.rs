...
#[test]
fn open_non_modelvault_file_returns_format_error() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("not_modelvault.modelvault");
...
fn open_wrong_magic_with_full_header_returns_format_error() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("bad_magic.modelvault");
...
fn open_unsupported_version_returns_format_error() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("bad_version.modelvault");
...
fn open_truncated_file_returns_format_error() {
    let dir = tempfile::tempdir().expect("tempdir");
    let path = dir.path().join("truncated.modelvault");
