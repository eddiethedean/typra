...
fn register_rejects_empty_name_after_trim() {
    ...
    let path = dir.path().join("e.modelvault");
...
fn register_trims_whitespace_around_name() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("trim.modelvault");
    let mut db = Database::open(&path).unwrap();
...
fn register_accepts_max_length_collection_name() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("max.modelvault");
    let mut db = Database::open(&path).unwrap();
...
fn register_rejects_name_longer_than_max() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("long.modelvault");
    let mut db = Database::open(&path).unwrap();
...
fn schema_version_chain_through_v3_reopens_clean() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("chain.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
...
fn multiple_collections_stable_ids_after_reopen() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("multi.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
    ...
    assert!(db
        .catalog()
        .get(modelvault_core::schema::CollectionId(3))
        .is_some());
}
