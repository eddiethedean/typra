//! 1.x on-disk backwards compatibility: 1.0-shaped `.modelvault` files must keep opening in 1.y.
//!
...
/// Build a file representative of ModelVault 1.0: minor 6, catalog v4, record v2 + v3, indexes, txn framing.
...
fn one_x_reads_1_0_representative_file() {
    ...
    let path = dir.path().join("rep.modelvault");
...
fn one_x_append_to_1_0_file_preserves_existing_rows() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("append.modelvault");
    write_1_0_representative(&path);
...
fn committed_1_0_fixture_opens_and_matches_live_encoder() {
    ...
    let dir = tempfile::tempdir().unwrap();
    let fresh = dir.path().join("fresh.modelvault");
    ...
    let copy = dir.path().join("fixture_copy.modelvault");
