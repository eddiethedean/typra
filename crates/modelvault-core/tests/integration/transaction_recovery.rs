...
use modelvault_core::{CollectionId, Database, OpenOptions, RecoveryMode};
...
fn transaction_inserts_visible_after_commit_and_reopen() {
    ...
    let path = dir.path().join("txn.modelvault");
    {
    ...
    let id = CollectionId(1);
    assert!(db
        .get(id, &modelvault_core::ScalarValue::String("a".into()))
        .unwrap()
        .is_some());
    assert!(db
        .get(id, &modelvault_core::ScalarValue::String("b".into()))
        .unwrap()
        .is_some());
}
...
fn nested_transaction_errors() {
    ...
    assert!(matches!(
        err,
        modelvault_core::DbError::Transaction(modelvault_core::TransactionError::NestedTransaction)
    ));
}
...
fn open_strict_rejects_uncommitted_txn_tail_and_autotruncate_recovers() {
    ...
    let len_after_corruption;
    {
        ...
        let begin = modelvault_core::txn::encode_txn_payload_v0(42);
        w.append(
            SegmentHeader {
...
fn open_strict_rejects_torn_segment_tail_and_autotruncate_recovers() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("torn_seg.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        db.register_collection("books", vec![title_field()], "title")
            .unwrap();
    ...
    assert!(matches!(
        e,
        modelvault_core::DbError::Format(_) | modelvault_core::DbError::Io(_)
    ));
...
fn unique_index_violation_inside_transaction_rolls_back_all_writes() {
    ...
    assert!(matches!(
        res,
        Err(modelvault_core::DbError::Schema(
            modelvault_core::SchemaError::UniqueIndexViolation
        ))
    ));
...
fn realistic_disk_workflow_transaction_indexed_query_reopen() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("realistic.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        ...
        let q = Query {
            ...
            predicate: Some(Predicate::And(vec![
                Predicate::Eq {
                    path: FieldPath(vec![Cow::Borrowed("status")]),
                    value: modelvault_core::ScalarValue::String("open".into()),
                },
                Predicate::Eq {
                    path: FieldPath(vec![Cow::Borrowed("sku")]),
                    value: modelvault_core::ScalarValue::String("SKU-A".into()),
                },
            ])),
            limit: Some(10),
            order_by: None,
    ...
    let db2 = Database::open(&path).unwrap();
    let got = db2
        .get(CollectionId(1), &modelvault_core::ScalarValue::Int64(1))
        .unwrap()
        .expect("row");
    assert_eq!(
        got.get("qty"),
