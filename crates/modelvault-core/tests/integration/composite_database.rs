...
fn insert_get_nested_object_list_enum_roundtrip() {
    ...
    let path = dir.path().join("comp.modelvault");
    let (id, _) = {
...
fn insert_optional_note_roundtrip() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("opt.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        let (cid, _) = db
            .register_collection("c", composite_fields(), "id")
...
fn insert_nonempty_list_violation() {
    let dir = tempfile::tempdir().unwrap();
    let mut db = Database::open(dir.path().join("ne.modelvault")).unwrap();
    let (id, _) = db
        .register_collection("c", composite_fields(), "id")
        .unwrap();
    let mut meta = BTreeMap::new();
...
fn reopen_preserves_composite_row() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("re.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        let (cid, _) = db
            .register_collection("c", composite_fields(), "id")
