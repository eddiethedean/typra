...
use modelvault_core::file_format::FileHeader;
...
fn open_rejects_unsupported_format_minor() {
    ...
    let path = dir.path().join("bad_minor.modelvault");
...
fn register_rejects_whitespace_only_primary_field() {
    let dir = tempfile::tempdir().unwrap();
    let mut db = Database::open(dir.path().join("pk.modelvault")).unwrap();
    let e = db
...
fn insert_rejects_non_pk_type_mismatch() {
    let dir = tempfile::tempdir().unwrap();
    let mut db = Database::open(dir.path().join("ty.modelvault")).unwrap();
    let (id, _) = db
...
fn replay_errors_on_nested_field_paths_in_schema() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("nest.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        let (id, _) = db
            .register_collection("c", vec![id_field(), title_field()], "id")
...
fn open_rejects_record_segment_with_short_payload_body() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("short_rec.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        let (id, _) = db
            .register_collection("c", vec![id_field(), title_field()], "id")
    ...
    let begin = modelvault_core::txn::encode_txn_payload_v0(999);
    w.append(
        SegmentHeader {
    ...
    .unwrap();
    let commit = modelvault_core::txn::encode_txn_payload_v0(999);
    w.append(
        SegmentHeader {
