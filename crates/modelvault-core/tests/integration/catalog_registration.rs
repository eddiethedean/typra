...
use modelvault_core::file_format::{decode_header, FileHeader, FILE_HEADER_SIZE};
use modelvault_core::schema::{FieldDef, Type};
use modelvault_core::Database;
use modelvault_core::DbError;
use modelvault_core::SchemaError;

fn title_field() -> FieldDef {
    FieldDef {
        path: modelvault_core::schema::FieldPath(vec![Cow::Owned("title".to_string())]),
        ty: Type::String,
        constraints: vec![],
    }
}
...
fn duplicate_collection_name_errors() {
    ...
    let path = dir.path().join("d.modelvault");
...
fn unknown_collection_id_errors() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("u.modelvault");
    let mut db = Database::open(&path).unwrap();
    db.register_collection("c", vec![title_field()], "title")
        .unwrap();
    let err = db.register_schema_version(modelvault_core::schema::CollectionId(99), vec![]);
    assert!(matches!(
        err,
...
fn replay_create_a_create_b_then_version_bump_a() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("order.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        ...
        db.register_collection(
            "b",
            vec![FieldDef {
                path: modelvault_core::schema::FieldPath(vec![Cow::Owned("id".to_string())]),
                ty: Type::Int64,
                constraints: vec![],
            }],
            "id",
    ...
    let b = db
        .catalog()
        .get(modelvault_core::schema::CollectionId(2))
        .unwrap();
    assert_eq!(a.name, "a");
...
fn register_schema_version_v2_then_reopen() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("v2.modelvault");
    {
        let mut db = Database::open(&path).unwrap();
        db.register_collection("c", vec![title_field()], "title")
            .unwrap();
...
fn lazy_header_bump_from_v0_3_to_v0_4_on_register() {
    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("legacy.modelvault");
...
fn corrupt_catalog_payload_errors() {
    use modelvault_core::publish::append_manifest_and_publish;
    ...
    use modelvault_core::superblock::{Superblock, SUPERBLOCK_SIZE};

    let dir = tempfile::tempdir().unwrap();
    let path = dir.path().join("bad.modelvault");
    ...
    store
        .write_all_at(
            modelvault_core::file_format::FILE_HEADER_SIZE as u64,
            &sb_a.encode(),
        )
        .unwrap();
    store
        .write_all_at(
            (modelvault_core::file_format::FILE_HEADER_SIZE + SUPERBLOCK_SIZE) as u64,
