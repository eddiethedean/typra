...
fn compact_to_preserves_rows_and_indexes() {
    ...
    let path = dir.path().join("src.modelvault");
    let compacted = dir.path().join("dst.modelvault");
    ...
    let q = modelvault_core::query::Query {
        collection: cid,
        predicate: Some(modelvault_core::query::Predicate::Eq {
            path: FieldPath(vec![Cow::Owned("tag".to_string())]),
...
fn compact_in_place_roundtrips() {
    ...
    let cid = modelvault_core::CollectionId(1);
    db.insert(
        cid,
        BTreeMap::from([
