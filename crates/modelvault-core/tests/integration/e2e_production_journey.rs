...
fn production_journey_register_insert_index_query_txn_reopen_schema_bump_compact_verify() {
    ...
    let path = dir.path().join("app.modelvault");
    ...
    let start = (modelvault_core::file_format::FILE_HEADER_SIZE
        + 2 * modelvault_core::superblock::SUPERBLOCK_SIZE) as u64;
    let _ = modelvault_core::segments::reader::scan_segments(&mut store, start).unwrap();
}
