...
- **Current release**: `0.14.0` (see [`CHANGELOG.md`](CHANGELOG.md)) — **ModelVault** rebrand from Typra
...
In addition, ModelVault should support **multiple storage/compute modes** (embedded single-file ergonomics):
...
- [`docs/guides/why_modelvault.md`](docs/guides/why_modelvault.md) (positioning) · [`docs/comparisons/`](docs/comparisons/index.md) (vs other embedded stores)
...
- **Python**: primary **`modelvault.models`** API plus **`fields_json`**; full **`Database`** surface including migrations, compaction, snapshots; **`modelvault.dbapi`** (PEP 249 read-only `SELECT` subset). See [`python/modelvault/README.md`](python/modelvault/README.md) and [`docs/reference/python_api.md`](docs/reference/python_api.md).
- **CI / coverage**: multi-OS Rust and Python CI; **`cargo llvm-cov`** with a **minimum line-coverage gate for `modelvault-core`** (see [`Makefile`](Makefile) `COVERAGE_MODELVAULT_CORE_LINES`); **`make check-full`** includes **`scripts/verify-doc-examples.sh`**.
...
- **Rust (core + public API)**: what lands in `modelvault-core` / `modelvault` / `modelvault-derive`
- **Python**: what lands in `python/modelvault` (bindings and (later) pure-Python helpers)
...
- **File header + format recognition** (new file writes header; existing file validates; truncated/non-modelvault errors).
...
- **CI / coverage**: multi-OS Rust+Python CI plus coverage artifacts (today the repo also enforces a **minimum `modelvault-core` line coverage** in CI—see [`Makefile`](Makefile); that gate did not exist at the 0.2.0 ship date).
...
- **Python**
  - No major new public API required; optionally expose a `modelvault.debug_*` module for introspection (if desired).
- **Definition of done**
...
- **Python**
  - **`modelvault.Database`**: **`open`**, **`register_collection(name, fields_json)`** (no primary-field argument yet), **`collection_names()`**; **`fields_json`** documented in [`python/modelvault/README.md`](python/modelvault/README.md).
...
- **Rust (`modelvault-core`)**
  ...
  - **Criterion** bench [`crates/modelvault-core/benches/query.rs`](crates/modelvault-core/benches/query.rs) and **`make bench`** (`get` vs indexed equality vs scan).
- **Python (`modelvault`)**
  ...
  - Docs: **[`docs/guides/python.md`](docs/guides/python.md)** (queries, indexes, DB-API design note); verified examples in **`scripts/verify-doc-examples.sh`**; integration tests under **`python/modelvault/tests/`**.
...
- **Typed subset models**: Rust `collection::<T>()` + Python `modelvault.models.collection` compatibility checks shipped in **1.0.x**; projection-aware decode remains **1.1**.
...
- **DB-API 2.0 (PEP 249)**: shipped as an experimental, read-only opt-in module (`modelvault.dbapi`) in **0.10.0** (minimal `SELECT` subset).
...
**Goal:** make ModelVault usable via standard Python DB tooling while keeping the engine’s typed query AST as the source of truth.

- **Rust**
  - Introduce a **text-to-Query adapter** (internal module) that parses a small read-only `SELECT`-style subset into the existing `modelvault-core` query AST.
...
- **Python**
  - Add `modelvault.dbapi` implementing **PEP 249** for the supported read-only subset:
...
- **Python**
  - Finalize typing story for DB-API rows and `modelvault.pyi` stability guarantees.
- **Definition of done**
...
**Delivered in 1.0.0**
- **`make check-full`**, **`verify-doc-examples`**, **`modelvault-core` line-coverage gate**, operational **`modelvault-cli`**, **`modelvault.models`**, multi-segment schema paths + record v3, compatibility/readiness docs.

#### 1.0 “must-have” (delivered)

These items were high-leverage for real applications shipping ModelVault as an embedded DB. **All listed below shipped in 1.0.0** unless noted as deferred to 1.1.0.

- **Operational CLI (`modelvault` binary)**:
  - `modelvault inspect <path>`: print header/minor, superblock generations, latest checkpoint offset, catalog summary (collections, schema versions, indexes).
  - `modelvault verify <path>`: run a read-only integrity scan (checksums, segment framing, catalog decode) with a clear exit code.
  - `modelvault dump-catalog <path> --json`: machine-readable catalog output for debugging/support.
...
- **Cross-process safety (file locking policy)**:
  - Prevent two writers from corrupting the same `.modelvault` file; document the concurrency contract.
...
- **Typed, model-first Python API stabilization**
  - Treat `modelvault.models` as the primary interface (dataclass + Pydantic v2), including constraints/indexes declarations, schema evolution (`plan/apply`), and patch updates.
  - Lock a public “v1” contract for `modelvault.models` (errors, naming rules, supported typing constructs).
  - Definition of done: parity tests for dataclass and Pydantic; docs are class-model-first; `modelvault.pyi` stable for the `models` surface.
...
- **Operational CLI**, **file locking**, **backup/restore**, **multi-segment paths + record v3**, **`modelvault.models`** stabilization, **tracing** feature, **compatibility/security/readiness docs**, and **CI gates** — see [`CHANGELOG.md`](CHANGELOG.md) **1.0.0** and [`docs/reference/readiness.md`](docs/reference/readiness.md).
...
- **Testing**
  ...
  - Invariant testing: **unique indexes** and **index vs scan** consistency have **started** in `modelvault-core` / Python tests—**expand** for txn boundaries, compaction, and multi-version schemas (**0.8+**).
- **Security**
...
- **Multi-language SDKs**
  - Goal: make ModelVault accessible from other application languages while sharing the Rust core.
  - Primary targets:
...
This section refines the roadmap to ensure ModelVault can operate **in memory and on disk** with embedded single-file ergonomics, and later support a **hybrid + streaming** approach for workloads that exceed RAM.
...
- **Hybrid buffered execution (future)**:
  - The database remains a normal **file-backed** ModelVault database.
...
- **Storage boundary** (in `modelvault-core`):
  ...
  - Provide at least two store implementations:
    - `FileStore`: segment/page IO over a `.modelvault` file.
...
- **Spill location**: default to **internal temporary segments** within the same `.modelvault` file (so “hybrid” still looks like a normal file-backed database to users). Consider a sidecar only if there is a compelling operational reason.
- **Snapshot format guarantee**: snapshots should be valid `.modelvault` files whenever possible (so the snapshot is not a special-case format).

## Subset models / projections (UI ergonomics)

For developer ergonomics—especially for collections with large, deeply nested schemas—ModelVault should support **subset models** (also known as projections or views).
